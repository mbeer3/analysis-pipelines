...
set -x
samtools sort -@ $ncpus -m 4G -O bam -T ${unfiltered_bam_root} $unfiltered_bam > ${unfiltered_bam_root}_sorted.bam
set +x
...
time java -jar ~/work/scripts/picard.jar MarkDuplicates \
    INPUT=${unfiltered_bam_root}_sorted.bam OUTPUT=${marked_bam_root}.bam METRICS_FILE=${filtered_bam_root}_dup_qc.txt \
